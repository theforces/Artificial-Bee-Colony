...
public:
	...
	double getResult(vector<double> numbers) {
		...
		/* rastrigin function */
			...
			double sumResult = 0;
			for (int i = 0; i < numbers.size(); i++) {
				sumResult += (pow(numbers[i], 2) - 10.0f * cos(2 * M_PI * numbers[i]) + 10.0f);
			}
			return sumResult;
		}
		/* rosenbrock function */
		...
		/* ackley function */
			...
			for (int i = 0; i < numbers.size(); i++) {
				...
				sumResult2 += cos(2 * M_PI * numbers[i]);
			}
			return 20.0f + M_E - 20.0f * pow(M_E, -0.2 * sqrt(1 / double(numbers.size()) * sumResult1)) - pow(M_E, 1 / double(numbers.size()) * sumResult2);
		}
		/* schwefel function */
		else if (m_choice == 5) {
			double sumResult = 0;
